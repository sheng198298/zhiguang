...
import com.jk.knowpost.api.dto.KnowPostContentConfirmRequest;
import com.jk.knowpost.api.dto.KnowPostDraftCreateResponse;
import com.jk.knowpost.api.dto.KnowPostPatchRequest;
import com.jk.knowpost.api.dto.KnowPostTopPatchRequest;
import com.jk.knowpost.api.dto.KnowPostVisibilityPatchRequest;
...
public class KnowPostController {
    ...
    /**
     * 关注 Feed（个性化推流时间线），游标分页；默认每页 20，最大 50。
     * 需要认证。
     */
    @GetMapping("/following")
    public FeedPageResponse following(@RequestParam(value = "cursor", required = false) String cursor,
                                      @RequestParam(value = "size", defaultValue = "20") int size,
                                      @AuthenticationPrincipal Jwt jwt) {
        long userId = jwtService.extractUserId(jwt);
        return feedService.getFollowingFeed(userId, size, cursor);
    }
