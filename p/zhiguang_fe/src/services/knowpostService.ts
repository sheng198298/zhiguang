...
export const knowpostService = {
  ...
  // 获取关注 Feed 列表（需鉴权，游标分页）
  following: (cursor?: string, size = 20) => {
    const params = new URLSearchParams({ size: String(size) });
    if (cursor) params.set("cursor", cursor);
    return apiFetch<FeedResponse>(`${KNOWPOST_PREFIX}/following?${params.toString()}`);
  }
  ,

  // 个性化推荐 Feed（需鉴权，游标分页）
  recommend: (cursor?: string, size = 20) => {
    const params = new URLSearchParams({ size: String(size) });
    if (cursor) params.set("cursor", cursor);
    return apiFetch<FeedResponse>(`${KNOWPOST_PREFIX}/recommend?${params.toString()}`);
  }
  ,

  // 热点榜单（日/周/总榜）
  hotlist: (window: "daily" | "weekly" | "alltime", page = 1, size = 20) =>
    apiFetch<HotListResponse>(`/api/v1/hot/${window}?page=${page}&size=${size}`)
  ,
